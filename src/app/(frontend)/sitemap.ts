...
/**
 * Every page is listed once per locale, and deliberately without an
 * `alternates.languages` block.
 *
 * Next turns that block into `<xhtml:link>` elements, which forces
 * `xmlns:xhtml` onto the root `<urlset>` — and a browser that finds elements
 * in the XHTML namespace stops pretty-printing the document and tries to
 * render them instead, which is what turned this file into one grey paragraph
 * of text rather than the collapsible tree every other sitemap shows.
 *
 * Nothing is lost to search: each page already carries the same hreflang set
 * as `<link rel="alternate">` in its head, via localeAlternates in
 * src/lib/i18n/metadata.ts. The sitemap copy was a second statement of the
 * same fact.
 */
